...
def index(request):
	...
	if HANDLER_ANY in request.session.get("current_user_role", [None]):
		...
		if HANDLER_COLL in request.session.get("user_roles", [None]) and not HANDLER_SENS in request.session.get("user_roles", [None]):
			...
			c0 = q.filter(id__in=Collection.objects.filter(customSecured__gt = 0, address__in = get_collections_where_download_handler(userId), status__gt = 0).values("request")).exclude(sensstatus=99)
