...
def usersettings(http_request):
    ...
    user_settings = AdminUserSettings.objects.filter(
        user=http_request.session["user_id"])
...
def save_user_settings(http_request):
    if http_request.method == 'POST':
        ...
        user_settings = AdminUserSettings.objects.get(
            user=http_request.session["user_id"])
        user_settings.emailNewRequests = http_request.POST.get(
            'email_new_requests', AdminUserSettings.NONE)
        user_settings.enableCustomEmailAddress = http_request.POST.get(
            'use_custom_email', False)
        user_settings.customEmailAddress = http_request.POST.get(
            'email_address', None)
        user_settings.changedBy = changed_by_session_user(http_request)
        user_settings.save()
        http_request.session["toast"] = {"status": toast.POSITIVE, "message": ugettext(
            'toast_user_settings_saved_succesfully')}
...
def save_pyha_settings(http_request):
    if http_request.method == 'POST':
        ...
        if not is_admin(http_request):
            return HttpResponse(status=404)
        pyha_settings = AdminPyhaSettings.objects.filter(
            settingsName='default')
        if pyha_settings.exists():
            pyha_settings = pyha_settings.first()
            pyha_settings.enableDailyHandlerEmail = http_request.POST.get(
                'enable_daily_handler_email', False)
            pyha_settings.enableDailyRequesterEmail = http_request.POST.get(
                'enable_daily_requester_email', False)
            pyha_settings.enableWeeklyMissingHandlersEmail = http_request.POST.get(
                'enable_weekly_missing_handlers_email', False)
            pyha_settings.enableDeclineOverdueCollections = http_request.POST.get(
                'enable_auto_decline_overdue', False)
            pyha_settings.changedBy = changed_by_session_user(http_request)
            pyha_settings.save()
            http_request.session["toast"] = {"status": toast.POSITIVE, "message": ugettext(
                'toast_pyha_settings_saved_succesfully')}
        return HttpResponseRedirect(nexturl)
    return HttpResponseRedirect(reverse('pyha:root'))
