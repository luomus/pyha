...
def send_email(http_request):
    if http_request.method == 'POST':
        ...
        content = http_request.POST.get(
            'com_email_content').replace(u'\ufeff', '')
        ...
        http_request.session["toast"] = {
            "status": toast.POSITIVE, "message": ugettext('toast_mails_sent_succesfully')}
        return HttpResponseRedirect(nexturl)
    return HttpResponse(status=404)
...
def answer(http_request):
    ...
    if http_request.method == 'POST':
        ...
        collection = Collection.objects.get(
            request=requestId, address=collectionId)
...
def group_answer(http_request):
    ...
    if http_request.method == 'POST':
        ...
        collections = Collection.objects.filter(request=requestId, id__in=[
                                                ide['id'] for ide in id_list], address__in=[ide['address'] for ide in id_list])
...
def question(http_request):
    ...
    if http_request.method == 'POST':
        ...
        if(int(http_request.POST.get('answer')) == 2):
            ...
            userRequest.changedBy = changed_by_session_user(http_request)
            userRequest.save()
            send_mail_after_additional_information_requested(
                requestId, userRequest.lang)
    return HttpResponseRedirect(nexturl)
...
def download_pdf(http_request):
    if http_request.method == 'POST':
        ...
        chat_entry = RequestInformationChatEntry.requestInformationChat.get(
            id=chat_entry_id)
        ...
        if not (
            ...
            is_allowed_to_ask_information_as_target(
                http_request, chat_entry.target, request_id)
        ):
            return HttpResponseRedirect(reverse('pyha:root'))
