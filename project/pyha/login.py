...
def log_in(http_request, token, authentication_info):
    ...
    if not "user_id" in http_request.session:
        ...
        if CAT_HANDLER_SENS in http_request.session["user_roles"] or CAT_HANDLER_COLL in http_request.session["user_roles"]:
...
def allowed_to_view(http_request, requestId, userId, role1, role2):
...
def is_allowed_to_ask_information_as_target(http_request, target, requestId):
    if target == 'admin':
        ...
        return CAT_HANDLER_SENS in http_request.session["user_roles"]
