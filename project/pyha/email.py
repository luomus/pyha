...
def send_mail_for_missing_handlers(collections_missing_handler, lang='fi'):
    ...
    with translation.override(lang):
        ...
        text_content = _get_email_content(
            'mail_collections_missing_handlers', lang, context)
...
def get_template_of_mail_for_approval(requestId, lang):
    ...
    with translation.override(lang):
        ...
        template = {'header': subject,
                    'content': text_content, 'sender': from_email}
        return template
...
def send_mail_after_additional_information_received(requestId, users, lang='fi'):
    ...
    with translation.override(lang):
        ...
        subject = ugettext(
            'mail_after_additional_information_received_subject')
        text_content = _get_email_content(
            'mail_after_additional_information_received', lang, context)
        from_email = settings.ICT_EMAIL
        to = [fetch_email_address(userId) for userId in users]
...
def send_mail_for_unchecked_requests(userId, request_ids, lang='fi'):
    ...
    send_mail_for_unchecked_requests_to_email(
        fetch_email_address(userId), request_ids, lang)


def send_mail_for_unchecked_requests_to_email(mailto, request_ids, lang='fi'):
    with translation.override(lang):
        req_links = [
            '{}request/{}'.format(settings.PYHA_URL, str(req_id)) for req_id in request_ids]
        ...
        text_content = _get_email_content(
            'mail_for_unchecked_requests', lang, context)
        from_email = settings.ICT_EMAIL
        to = [mailto]
...
def send_admin_mail_after_approved_request(requestId, mailto, lang='fi'):
    ...
    with translation.override(lang):
        ...
        subject = ugettext(
            'mail_admin_after_request_approval_subject').format(**context)
        text_content = _get_email_content(
            'mail_admin_after_request_approval', lang, context)
        from_email = settings.PYHA_EMAIL
        to = [mailto]
...
def send_admin_mail_after_approved_request_missing_handlers(requestId, mailto, lang='fi'):
    ...
    with translation.override(lang):
        ...
        subject = ugettext(
            'mail_admin_after_request_approval_missing_handlers_subject').format(**context)
        text_content = _get_email_content(
            'mail_admin_after_request_approval_missing_handlers', lang, context)
        from_email = settings.PYHA_EMAIL
        to = [mailto]
