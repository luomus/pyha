...
def decline_in_delay(days, collection, interval):
    ...
    if(is_collection_waiting_atleast_days(days, collection)):
        ...
        collection.save()
        RequestLogEntry.requestLog.create(request=Request.objects.get(id=collection.request.id), collection=collection,
                                          user="Laji.fi ICT-team", role=CAT_HANDLER_COLL, action=RequestLogEntry.DECISION_NEGATIVE_OVERDUE)
