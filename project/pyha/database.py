...
def create_collections_for_lists(requestId, http_request, collectionList, userRequest, userId):
    collectionList += Collection.objects.filter(
        request=userRequest.id, status__gte=0)
...
def create_new_contact(http_request, userRequest, count):
    ...
    contact.personName = http_request.POST.get(
        'request_person_name_'+str(count))
    contact.personStreetAddress = http_request.POST.get(
        'request_person_street_address_'+str(count))
    contact.personPostOfficeName = http_request.POST.get(
        'request_person_post_office_name_'+str(count))
    contact.personPostalCode = http_request.POST.get(
        'request_person_postal_code_'+str(count))
    contact.personCountry = http_request.POST.get(
        'request_person_country_'+str(count))
    contact.personEmail = http_request.POST.get(
        'request_person_email_'+str(count))
    contact.personPhoneNumber = http_request.POST.get(
        'request_person_phone_number_'+str(count))
    contact.personOrganizationName = http_request.POST.get(
        'request_person_organization_name_'+str(count))
    contact.personCorporationId = http_request.POST.get(
        'request_person_corporation_id_'+str(count))
...
def update_contact_preset(http_request, userRequest):
    ...
    contactPreset.requestPersonName = http_request.POST.get(
        'request_person_name_1')
    contactPreset.requestPersonStreetAddress = http_request.POST.get(
        'request_person_street_address_1')
    contactPreset.requestPersonPostOfficeName = http_request.POST.get(
        'request_person_post_office_name_1')
    contactPreset.requestPersonPostalCode = http_request.POST.get(
        'request_person_postal_code_1')
    contactPreset.requestPersonCountry = http_request.POST.get(
        'request_person_country_1')
    contactPreset.requestPersonEmail = http_request.POST.get(
        'request_person_email_1')
    contactPreset.requestPersonPhoneNumber = http_request.POST.get(
        'request_person_phone_number_1')
    contactPreset.requestPersonOrganizationName = http_request.POST.get(
        'request_person_organization_name_1')
    contactPreset.requestPersonCorporationId = http_request.POST.get(
...
def get_unhandled_requests_data(userId):
    ...
    collection_list = Collection.objects.filter(
        address__in=user_collections, status=StatusEnum.WAITING)
    ...
    for r in request_list:
        request_collections = collection_list.filter(
            request=r.id).values_list("address", flat=True)
...
def ignore_official_database_update_request_status(wantedRequest, lang):
    ...
    accepted, declined, pending = get_collection_status_counts(
        wantedRequest.id)
...
def handler_mul_information_chat_answered_status(request_list, http_request, userId):
    ...
    if CAT_HANDLER_COLL in http_request.session.get("user_roles", [None]):
        ...
        coobList = list(Collection.objects.filter(request__in=[
                        re.id for re in request_list], address__in=get_collections_where_download_handler(userId)))
        for r in request_list:
            colist = [co for co in coobList if co.request_id == r.id]
            for co in colist:
                colbasedchatlist = [
                    x for x in reqFilteredColChatList if x.target == co.address and x.request_id == r.id]
...
def create_request_view_context(requestId, http_request, userRequest):
    ...
    create_collections_for_lists(
        requestId, http_request, collectionList, userRequest, userId)
    for collection in collectionList:
        collection.counts = get_collection_counts(
            collection, http_request.LANGUAGE_CODE)
    ...
    if role == ADMIN:
        ...
        context["com_email_template"] = get_template_of_mail_for_approval(
            userRequest.id, 'fi')
    ...
    if userRequest.status != StatusEnum.APPROVETERMS_WAIT:
        ...
        isEndable = (Collection.objects.filter(
            request=userRequest.id, status=4).exists())
    ...
    if userRequest.status == StatusEnum.APPROVETERMS_WAIT and Request.objects.filter(user=userId, status__gte=1).count() > 0:
        ...
        context["requestHandlerChat_list"] = requestHandlerChat(
            http_request, userRequest)
        requestInformationChat_list = requestInformationChat(
            http_request, userRequest, userId)
...
def get_reasons(userRequest):
    if(userRequest.reason != None):
        reasonlist = json.loads(
            userRequest.reason, object_hook=lambda d: Namespace(**d))
...
def make_logEntry_view(http_request, userRequest, userId, role):
    ...
    if userRequest.id not in http_request.session.get("has_viewed", [None]):
        ...
        RequestLogEntry.requestLog.create(
            request=userRequest, user=userId, role=logRole, action=RequestLogEntry.VIEW)


def requestLog(http_request, requestId):
    requestLog_list = list(RequestLogEntry.requestLog.filter(
        request=requestId).order_by('-date'))
    collectionList = []
    email = []
...
def requestInformationChat(http_request, userRequest, userId):
    requestInformationChat_list = list(RequestInformationChatEntry.requestInformationChat.filter(
        request=userRequest).order_by('date'))
...
def get_last_information_chat_entries(user_id, current_roles):
    ...
    if ADMIN not in current_roles:
        collection_filter_query_string, context2 = _get_collection_filter_query_string_and_context(
            user_id)
...
def emailsOnUpdate(pending, userRequest, lang):
    if pending == 0:
        send_mail_after_request_has_been_handled_to_requester(
            userRequest.id, lang)
    else:
        # Send email if status changed
        send_mail_after_request_status_change_to_requester(
            userRequest.id, lang)
...
def save_request_sent_status_email(request, accepted, declined, pending):
    RequestSentStatusEmail.objects.create(
        request=request, accepted_count=accepted, declined_count=declined, pending_count=pending)
