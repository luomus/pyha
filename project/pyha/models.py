...
class TruncatingReasonJsonCharField(models.CharField):
    def get_prep_value(self, value):
        ...
        value = super(TruncatingReasonJsonCharField,
...
class Collection(models.Model):
    ...
    downloadRequestHandler = models.CharField(
        max_length=500, blank=True, null=True)
    decisionExplanation = TruncatingTextField(
...
# Used currently for the admin email gatekeeper, on who has been emailed per request
class HandlerInRequest(models.Model):
...
# Keep track of the last status email that has been sent to user
class RequestSentStatusEmail(models.Model):
...
class Request(models.Model):
    ...
    # description given by the requester for his request
    description = TruncatingCharField(max_length=400, blank=True, null=True)
    ...
    personStreetAddress = TruncatingCharField(
        max_length=100, blank=True, null=True)
    personPostOfficeName = TruncatingCharField(
        max_length=100, blank=True, null=True)
    personPostalCode = TruncatingCharField(
        max_length=100, blank=True, null=True)
    ...
    personPhoneNumber = TruncatingCharField(
        max_length=100, blank=True, null=True)
    personOrganizationName = TruncatingCharField(
        max_length=100, blank=True, null=True)
    personCorporationId = TruncatingCharField(
        max_length=100, blank=True, null=True)
    reason = TruncatingReasonJsonCharField(
...
class RequestLogEntry(models.Model):
    ...
    ACTION = (
        ...
        (DECISION_POSITIVE_OVERDUE,
         'accepted use of data, because decision has been overdue'),
        (DECISION_POSITIVE_NO_OBSERVATIONS,
         'accepted use of data, because no observations'),
        (DECISION_RESET, 'resets the decision regarding data'),
        (DECISION_NEGATIVE, 'declines use of data'),
        (DECISION_NEGATIVE_OVERDUE,
         'declines use of data, because decision has been overdue'),
...
class ContactPreset(models.Model):
    ...
    requestPersonName = TruncatingCharField(
        max_length=100, blank=True, null=True)
    requestPersonStreetAddress = TruncatingCharField(
        max_length=100, blank=True, null=True)
    requestPersonPostOfficeName = TruncatingCharField(
        max_length=100, blank=True, null=True)
    requestPersonPostalCode = TruncatingCharField(
        max_length=100, blank=True, null=True)
    requestPersonCountry = TruncatingCharField(
        max_length=100, blank=True, null=True)
    requestPersonEmail = TruncatingCharField(
        max_length=100, blank=True, null=True)
    requestPersonPhoneNumber = TruncatingCharField(
        max_length=100, blank=True, null=True)
    requestPersonOrganizationName = TruncatingCharField(
        max_length=100, blank=True, null=True)
    requestPersonCorporationId = TruncatingCharField(
        max_length=100, blank=True, null=True)
    changedBy = models.CharField(max_length=100)
    history = HistoricalRecords()
...
class AdminUserSettings(models.Model):
    ...
    user = models.CharField(primary_key=True, max_length=100)
    emailNewRequests = models.CharField(
        max_length=10, choices=EMAIL_NEW_REQUESTS_SETTING, default=NONE)
    enableCustomEmailAddress = models.BooleanField(default=False)
    customEmailAddress = TruncatingCharField(
        max_length=100, blank=True, null=True)
    changedBy = models.CharField(max_length=100)
    history = HistoricalRecords()
